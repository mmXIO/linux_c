...
#默认源码安装的软件没有service文件，无法通过systemd管理
#手动编写service文件，方便在CentOS7的环境中管理服务
function conf_nginx_systemd() {
...
#Note:需要提前到官网下载带boost版本的MySQL(mysql-boost-8.0.13.tar.gz)
function install_mysql8() {
  ...
  cd mysql-8.0.13/
  $SETCOLOR_WARNING
  echo "请确保有超过2GB的可用内存,否则编译可能出错,并提示Killed(程序cclplus)"
...
-DSYSCONFDIR=/etc/ -DMYSQL_UNIX_ADDR=/tmp/mysql.sock
  ...
  #DWITH_FEDERATED_STORAGE_ENGIN：开启FEDERATED存储引擎,支持远程数据库
  ...
  cat > /etc/ld.si.conf.d/mysql.conf <<- EOF
  /usr/local/mysql/lib
EOF
  cd ..
}

function init_mysql8() {
